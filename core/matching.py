...
class TextBoxMatcher:
    ...
    def match_boxes(self, jp_key: str, cn_key: str, jp_size: tuple, cn_size: tuple) -> list:
        ...
        # KD树匹配
        matches = self._kd_match(jp_boxes, cn_boxes_converted)
    ...
    def _kd_match(self, jp_boxes: list, cn_boxes: list) -> list:
        """KD树匹配核心算法"""
        if not jp_boxes or not cn_boxes:
            return []
            
        jp_y_centers = np.array([(box[1]+box[3])/2 for box in jp_boxes]).reshape(-1, 1)
        tree = cKDTree(jp_y_centers)
        
        matched_jp = set()
        matches = []
        
        for cn_idx, cn_box in enumerate(cn_boxes):
            cy = (cn_box[1] + cn_box[3])/2
            candidates = tree.query_ball_point([[cy]], self.y_threshold)[0]
            
            # 筛选未匹配的候选项
            valid_candidates = [idx for idx in candidates if idx not in matched_jp]
            if not valid_candidates:
                continue
                
            # X轴最近邻匹配
            cx_cn = (cn_box[0] + cn_box[2])/2
            closest = min(valid_candidates, 
                         key=lambda idx: abs((jp_boxes[idx][0]+jp_boxes[idx][2])/2 - cx_cn))
            
            matched_jp.add(closest)
            matches.append((closest, cn_idx))
...
def create_new_masks(match_result, raw_mask_dir, output_dir):
    ...
    for page_name, page_entries in match_result["pages"].items():
        ...
        if not raw_image_path:
            print(f"警告: 未找到生肉图片: {page_name}")
            continue
        
        # 查找对应的mask文件
        mask_path = Path(raw_mask_dir) / f"mask-{page_name}.png"
        if not mask_path.exists():
            print(f"警告: 未找到mask文件: {mask_path}")
            continue
        ...
        try:
            ...
            print(f"✓ 已创建新mask: {output_path.name}")
            
        except Exception as e:
            print(f"✗ 创建页面 {page_name} 的新mask时出错: {str(e)}")
...
def match_and_create_masks(raw_annotations_path, text_annotations_path, output_path, 
    ...
    for page_idx, page_name in enumerate(raw_pages):
        ...
        if page_name not in text_json:
            print(f"警告: 熟肉中未找到页面 {page_name}，跳过")
        ...
        try:
            ...
            print(f"获取图片尺寸失败 {page_name}: {e}")
            result["pages"][page_name] = []
            continue
        
        print(f"页面 {page_name}: 生肉尺寸 {raw_size}, 熟肉尺寸 {text_size}")
        ...
        print(f"页面 {page_name}: 生肉 {len(raw_boxes)} 个框, 熟肉 {len(text_boxes)} 个框")
        ...
        print(f"页面 {page_name}: 匹配 {matched_count}/{len(page_results)} 个文本框")
